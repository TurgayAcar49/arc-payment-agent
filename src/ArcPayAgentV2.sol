...
contract ArcPayAgentV2 {
    ...
    function createRule(address target, uint256 threshold, uint256 amount) external onlyOwner returns (uint256 ruleId) {
        ...
        rules[ruleId] = PaymentRule({target: target, threshold: threshold, amount: amount, active: true});

        emit RuleCreated(ruleId, target, threshold, amount);
    }

    function executeRule(uint256 ruleId) external onlyOwner {
